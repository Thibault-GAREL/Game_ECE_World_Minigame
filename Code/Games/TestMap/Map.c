...
void Map_Create(PGAME _pMap)                               // ECHELLE = 1.25 POUR LE MOMENT
{
    ...
    pMapData->police[1]= al_load_ttf_font("..\\Textures/Map/police.ttf",50,0);
    ...
    pMapData->image[49]= al_load_bitmap("..\\Textures/Map/selectgame.png");
    pMapData->image[50]= al_load_bitmap("..\\Textures/Map/AfficheTDLR.png");
    pMapData->image[51]= al_load_bitmap("..\\Textures/Map/AfficheTAB.png");
    ...
    pMapData->image[59]= al_load_bitmap("..\\Textures/Map/AfficheGEO.jpg");
...
void affichageminimap(PGAME _pMap){
    ...
    if (pMapData->jeusuivant == GAME_GEO){
        if (pMapData->bonhommex > pMapData->pimages[6].x-pMapData->x+1449 && pMapData->bonhommex < pMapData->pimages[6].x-pMapData->x+1496 && pMapData->bonhommey > pMapData->pimages[6].y-pMapData->y+927 && pMapData->bonhommey < pMapData->pimages[6].y-pMapData->y+987){
            pMapData->compteurfin=1;
        }
        else {
            al_draw_bitmap(pMapData->image[14],1559,865,0);
            al_draw_rectangle(1535+(pMapData->x)/19,930+(pMapData->y)/20,1535+(pMapData->x)/19+2,890,al_map_rgb(0,255,228),2);
            al_draw_rectangle(1570,890,1535+(pMapData->x)/19,892, al_map_rgb(0,255,228),2);
        }
    }
...
void tableaudescore(PGAME _pMap){
    ...
    al_draw_text(pMapData->police[1], al_map_rgb(0,0,0),600,500,0,_pMap->pPlayers[0]->name);
    al_draw_text(pMapData->police[1], al_map_rgb(0,0,0),600,580,0,_pMap->pPlayers[1]->name);
...
void choisirjeusuivant(PGAME _pMap){
    MapData* pMapData = _pMap->gameData;
    ...
    al_draw_filled_rectangle(947,750,947+247,950, al_map_rgb(0,0,0));
    ...
    al_draw_bitmap(pMapData->image[59],947,750,0);
    ...
    if (pMapData->mouse.x >= 447 && pMapData->mouse.x < 694 && pMapData->mouse.y >= 500 && pMapData->mouse.y <= 700 && pMapData->click==1){
        pMapData->jeusuivant=GAME_TAB;
        pMapData->compteurchoixjeu=1;
    }
    if (pMapData->mouse.x >= 694 && pMapData->mouse.x < 941 && pMapData->mouse.y >= 500 && pMapData->mouse.y <= 700 && pMapData->click==1){
        pMapData->jeusuivant=GAME_SNAKE;
        pMapData->compteurchoixjeu=1;
    }
    if (pMapData->mouse.x >= 941 && pMapData->mouse.x < 1188 && pMapData->mouse.y >= 500 && pMapData->mouse.y <= 700 && pMapData->click==1){
    ...
    if (pMapData->mouse.x >= 947 && pMapData->mouse.x < 947+247 && pMapData->mouse.y >= 750 && pMapData->mouse.y < 950 && pMapData->click==1){
        pMapData->jeusuivant=GAME_GEO;
        pMapData->compteurchoixjeu=1;
    }
}
